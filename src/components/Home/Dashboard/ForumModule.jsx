...
const ForumModule = ({ darkMode }) => {
  ...
  const fetchAnswers = async (questionId) => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await axios.get(`${API_URL}/api/forum/questions/${questionId}/answers`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAddAnswer = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const response = await axios.post(
        `${API_URL}/api/forum/questions/${selectedQuestion.id}/answers`,
        {
          content: newAnswer
        },
        {
  ...
  const handleDeleteAnswer = async (answerId) => {
    ...
    try {
      ...
      if (selectedQuestion) {
        setQuestions(questions.map(q => 
          q.id === selectedQuestion.id 
            ? { ...q, answer_count: Math.max(0, q.answer_count - 1) }
            : q
        ));
      }
    } catch (error) {
